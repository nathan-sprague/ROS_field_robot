...
def findDiffSpeeds(currentCoords, targetCoords, currentHeading, targetHeading, finalHeading = False, turnConstant = 0, destTolerance = 5, obstacles=[]):
    ...
    # print("returning speeds:", fasterSpeed, slowerSpeed)
...
def calc_control_command(currentCoords, targetCoords, currentHeading, targetHeading, finalHeading = False, turnConstant = 0, destTolerance = 5, obstacles=[]):
    # x_diff, y_diff, theta, theta_goal):

    theta = math.radians(currentHeading)
    theta_goal = math.radians(targetHeading)

    x_diff, y_diff = findDistBetween(currentCoords, targetCoords)

    Kp_rho, Kp_alpha, Kp_beta = 9, 15, 3
...
#    return rho, v, w
    ...
    # v=-v
    # w=-w
    fasterSpeed = np.sign(v) * min(abs(v), 100)
    print("fasterSpeed", fasterSpeed)

    if rho < 1:
        fasterSpeed = 30

    print("w", w)
    w = np.sign(w) * min(abs(w), math.pi) * 0.01 # 0.01 is to decrease it
    slowerSpeed = fasterSpeed * w / math.pi
    ...
    print("returning speeds", robotSpeed)
    # robotSpeed = [robotSpeed[0]*-1, robotSpeed[1]*-1]
    return robotSpeed
