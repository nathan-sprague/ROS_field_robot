...
_runMode = 3 # 0=Real, 1=Simulation with sim video, 2=simulation with recorded video, 3=simulation with recorded video and position, 4=full playback but still process, 5=full playback
...
if _runMode != 0:
	...
	simulationFileName = "/home/nathan/new_logs/endurance2/log_1664920373"#"/home/nathan/new_logs/july29/morning/log_1659107371"
...
# change to True to use during navigation at times other than interrow navigation (keep true to log data - logs are saved in camera recordings)
...
navDestinations = exampleDests.endurancePath # destinations the robot will go to
...
class Robot:
	...
	def __init__(self, runMode=0):
		...
		self.threadStatus = {} # dictionary with all the threads and their statuses. key is thread name, value is time updated
		...
		self.inRowTime = 0 # time spent inside a row (to know wheter the robot was really in the row when it's about to exit)
		...
		# will add back eventually
		self.subPoints = [] # intermediate points between the robot's current location and destination to allow obstacle avoidance 
		...
		if self.runMode==0: # not testing
			...
			self.logFileName = "log_" + str(self.startTime)
			self.cam = video_navigation.RSCamera(self, saveVideo=False, filename=simulationFileName, 
						navMethod=0, startFrame=0, playbackLevel=self.runMode)
			print("made cam")

			if self.runMode < 4: # simulation
	...
	def navigate(self, destinations):
		...
		if "rows" in destinations[0]:
			print("there is row info\n\n")
			self.rows = destinations[0]["rows"]
			self.rowsID = random.randint(0,1000)

		if not "coord" in destinations[0]: # there was the obstacles and/or rows info in the first element of the destinations list
		...
		while self.notCtrlC and len(self.destinations) > 0:
			if time.time() - lastLogTime > 4:
				try:
					with open(self.logFileName + "/times.txt", 'a') as fileHandle:
						fileHandle.write(string(self.gpsUnixTime) + " = " + str(time.time()))
						fileHandle.close()
				except:
					print("Error logging time")
				lastLogTime = time.time()


			# if self.realSpeed[0] == self.realSpeed[1] and self.realSpeed[0] != 0:
			# 	print("big problem!!!!\n\n")
			# 	exit()
	...
	def interRowNavigate(self, rowInfo = False):
		...
		# deal with obstacles
		delayTime = self.processRowObstacles(camHeading)
		if delayTime != -1:
			return delayTime
		...
		if self.cam.outsideRow:
			...
			if self.cam.distFromCorn > 4000: # be willing to make sharp turns if far from corn entrance
				...
				if self.rowDirection != -1 and abs(self.trueHeading-self.rowDirection) > 2: # not headed in right direction
					



					# doing this with a bunch of if statements. Make more simplified later!!!!!!1
					rowAngle = self.rowDirection-self.trueHeading
					camAngle = -camHeading

					turnAmount = rowAngle * 5
					if abs(turnAmount) > 40:
						turnAmount = math.copysign(40, turnAmount)

					if rowAngle > 10: # already doing sharp left turn to get centered
						if camAngle*2 > rowAngle: # turned enough, straighten out
							dirAngle = -1
						else:
							dirAngle = 0

					elif rowAngle < -10: # already doing sharp right turn to get centered
						if camAngle*2 < rowAngle: # turned enough, straighten out
							dirAngle = 1
						else:
							dirAngle = 0
						...
						print("wasnt doing sharp turn before")
						dirAngle = 1
						if camAngle<rowAngle: # sharp left
							dirAngle = math.copysign(1, -rowAngle)
						elif camAngle>rowAngle: # sharp right
							dirAngle = math.copysign(1, rowAngle)


					travelDirection += turnAmount*dirAngle # add 10 degrees in same direction as travel direction
					if dirAngle > 0:
						print("sharp right")
					elif dirAngle < 0:
						print("sharp left")
						...
						print("go straight")
					print(rowAngle, camAngle, "heading off!\n\n\n\n")

			elif self.rowDirection != -1 and abs(self.trueHeading-self.rowDirection) > 10: # close to corn. Better be facing the right row direction
				if self.rowDirection>self.trueHeading:
					travelDirection = 60
				else:
					travelDirection = -60
	...
	def processRowObstacles(self, camHeading):
		if self.cam.aboutToHit:
			...
			if len(self.motionType)==2 and camHeading>0:
				self.bounces += 1
				self.motionType += [1]
			elif len(self.motionType) == 2: # did not add the direction yet (first time this if statement is called)
				self.motionType += [-1]
				self.bounces += 1
			return 0.3
		...
		elif self.motionType[0] == "reversing" and time.time()-self.motionType[1] < 10: # back up and turn a little bit to line up with target heading
		...
		elif self.motionType[0] == "reversing" and time.time()-self.motionType[1] < 15: # just turn to face heading
			if self.motionType[2] > 0:
